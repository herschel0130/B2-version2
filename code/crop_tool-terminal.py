...
class FITSCropTool:
    ...
    def confirm_crop(self, event):
        try:
            ...
            # 添加到历史记录
            fit_stats = getattr(self, 'current_fit_stats', {'fit_score': 0, 'reduced_chi_square': np.inf})
    ...
    def update_record_display(self):
        ...
        # 显示标题
        self.ax_record.text(0.05, 0.98, 'Recent Crops:', 
                           transform=self.ax_record.transAxes,
        ...
        for i, record in enumerate(reversed(recent_crops), 1):
            ...
            # 3行显示
            text = (f"#{len(self.crop_history) - len(recent_crops) + i}:\n"
                   f"TL:({x1},{y1})\n"
                   f"BR:({x2},{y2})\n"
                   f"χ²ᵣ={chi2:.2f}\n"
                   f"S={score:.0f}")
